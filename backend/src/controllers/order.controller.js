...
exports.createOrder = async (req, res, next) => {
  try {
    ...
    const {
      type, product_id, description, quantity,
      dimensions, material, estimatedBudget, incoterm
    } = req.body;
    ...
    if (type === 'existing_product' && !product_id) {
      return badRequest(res, 'product_id is required for existing_product orders');
    }
    ...
    const [result] = await pool.query(
      `INSERT INTO orders 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
      [clientId, product_id || null, type, description, quantity, dimensions, material, estimatedBudget, incoterm, attachmentPdf]
    );

    // Notify admin
    await _notifyUser(null, 'admin', `Nouvelle commande #${result.insertId}`, `Un client a créé une nouvelle commande.`);

    return created(res, { orderId: result.insertId }, 'Order created successfully');
  } catch (err) {
    next(err);
  }
};
...
exports.getOrderById = async (req, res, next) => {
  try {
    ...
    if (role === 'supplier'    && order.supplier_id     !== userId) return forbidden(res);
    if (role === 'transport'   && order.transport_id    !== userId) return forbidden(res);
    if (role === 'transitaire' && order.transitaire_id  !== userId) return forbidden(res);
...
exports.assignSupplier = async (req, res, next) => {
  try {
    ...
    // Notify supplier
    await _notifyUser(supplier_id, null, `Nouvelle demande #${id}`, `Une commande vous a été assignée.`);
...
exports.addSupplierQuote = async (req, res, next) => {
  try {
    ...
    const [order] = await pool.query('SELECT * FROM orders WHERE id = ? AND supplier_id = ?', [id, supplierId]);
    if (order.length === 0) return notFound(res, 'Order not found or not assigned to you');
    ...
    await pool.query(`UPDATE orders SET status = 'pending_transport' WHERE id = ?`, [id]);

    // Notify admin
    await _notifyUser(null, 'admin', `Devis fournisseur #${id}`, `Le fournisseur a soumis son devis.`);
...
exports.assignTransport = async (req, res, next) => {
  try {
    ...
    await _notifyUser(transport_id, null, `Nouvelle commande #${id}`, `Une commande vous a été assignée.`);
...
exports.addTransportQuote = async (req, res, next) => {
  try {
    ...
    const [order] = await pool.query('SELECT * FROM orders WHERE id = ? AND transport_id = ?', [id, transportId]);
    if (order.length === 0) return notFound(res, 'Order not found or not assigned to you');
    ...
    await pool.query(`UPDATE orders SET status = 'pending_transitaire' WHERE id = ?`, [id]);
    await _notifyUser(null, 'admin', `Devis transport #${id}`, `Le transporteur a soumis son devis.`);
...
exports.assignTransitaire = async (req, res, next) => {
  try {
    ...
    await _notifyUser(transitaire_id, null, `Nouvelle commande #${id}`, `Une commande vous a été assignée.`);
...
exports.addTransitaireQuote = async (req, res, next) => {
  try {
    ...
    const [order] = await pool.query('SELECT * FROM orders WHERE id = ? AND transitaire_id = ?', [id, transitaireId]);
    if (order.length === 0) return notFound(res, 'Order not found or not assigned to you');
    ...
    await pool.query(`UPDATE orders SET status = 'final_calculation' WHERE id = ?`, [id]);
    await _notifyUser(null, 'admin', `Devis transitaire #${id}`, `Le transitaire a soumis ses frais douaniers.`);
...
exports.calculateTotal = async (req, res, next) => {
  try {
    ...
    const [sq]  = await pool.query('SELECT productionCost FROM supplier_quotes  WHERE order_id = ?', [id]);
    const [tq]  = await pool.query('SELECT transportCost  FROM transport_quotes  WHERE order_id = ?', [id]);
    const [trq] = await pool.query('SELECT customsCost    FROM transitaire_quotes WHERE order_id = ?', [id]);

    if (!sq.length || !tq.length || !trq.length) {
      return badRequest(res, 'All quotes (supplier, transport, transitaire) must be submitted first');
    }
    ...
    await pool.query(`UPDATE orders SET status = 'pending_payment' WHERE id = ?`, [id]);

    // Notify client
    const [order] = await pool.query(
      'SELECT o.client_id, u.email, u.fullName FROM orders o JOIN users u ON o.client_id = u.id WHERE o.id = ?', [id]
    );
    if (order.length) {
      await _notifyUser(order[0].client_id, null, `Devis final prêt #${id}`, `Votre devis final est disponible.`);
      await sendOrderStatusEmail(order[0].email, order[0].fullName, id, 'Devis final prêt');
    }

    return success(res, { productionCost, transportCost, customsCost, mgtsMargin: margin, totalAmount });
  } catch (err) {
    next(err);
  }
};
...
exports.respondToQuote = async (req, res, next) => {
  try {
    ...
    if (order.length === 0) return notFound(res, 'Order not found or not in pending_payment status');
    ...
    if (decision === 'accept') {
      await _notifyUser(null, 'admin', `Devis accepté #${id}`, `Le client a accepté le devis.`);
    }

    return success(res, null, decision === 'accept' ? 'Quote accepted. Proceed to payment.' : 'Quote rejected.');
  } catch (err) {
    next(err);
  }
};
...
const _notifyUser = async (userId, role, title, content) => {
  try {
    if (userId) {
      ...
      const [admins] = await pool.query('SELECT id FROM users WHERE role = ?', [role]);
